...
class AdvancedGenZDetector:
    ...
    def extract_linguistic_features(self, text: str) -> Dict[str, float]:
        ...
        features["slang_patterns"] = len(re.findall(r"\b(yeet|no cap|bet|periodt|slay|vibe|stan|ship)\b", text.lower()))
        features["emoji_count"] = len(re.findall(r"[^\x00-\x7F]", text))
    ...
    def predict(self, title: str = "", content: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            if malicious_score > safe_score:
                ...
                final_confidence = np.average([c for p, c in zip(predictions, confidences) if p == "malicious"], 
                                            weights=[w for p, w in zip(predictions, weights) if p == "malicious"])
            else:
                final_prediction = "safe"
                final_confidence = np.average([c for p, c in zip(predictions, confidences) if p == "safe"], 
