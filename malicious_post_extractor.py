...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('malicious_extraction.log'),
...
class MaliciousPostExtractor:
    ...
    def __init__(self, model_path: str = "genz_detector_model.pkl", 
    ...
    def load_reddit_dataset(self, use_local: bool = False, local_file: str = None) -> pd.DataFrame:
        """
    ...
    def extract_malicious_posts(self, df: pd.DataFrame, 
                              min_posts: int = 1000, 
                              max_posts: int = 5000) -> pd.DataFrame:
        """
        ...
        if self.stats['malicious_posts_found'] < min_posts:
            logger.warning(f"Only found {self.stats['malicious_posts_found']} malicious posts, "
                          f"below minimum target of {min_posts}")
    ...
    def save_results(self, malicious_df: pd.DataFrame, 
                    output_file: str = "malicious_posts.csv") -> None:
        """
...
"""
    ...
    def run_extraction(self, min_posts: int = 1000, max_posts: int = 5000,
                      output_file: str = "malicious_posts.csv") -> pd.DataFrame:
        """
...
def main():
    ...
    parser.add_argument("--use-local", action="store_true", 
                       help="Use local CSV file instead of Hugging Face")
    parser.add_argument("--local-file", type=str, 
                       help="Path to local CSV file")
    parser.add_argument("--min-posts", type=int, default=1000,
                       help="Minimum number of malicious posts to extract")
    parser.add_argument("--max-posts", type=int, default=5000,
                       help="Maximum number of malicious posts to extract")
    parser.add_argument("--confidence", type=float, default=0.7,
                       help="Confidence threshold for malicious classification")
    parser.add_argument("--output", type=str, default="malicious_posts.csv",
    ...
    try:
        ...
        extractor = MaliciousPostExtractor(
            model_path="genz_detector_model.pkl",
            confidence_threshold=args.confidence
        )
        
        # Load dataset
        df = extractor.load_reddit_dataset(
            use_local=args.use_local,
            local_file=args.local_file
        )
        
        # Extract malicious posts
        malicious_posts = extractor.extract_malicious_posts(
            df, 
