...
class AdvancedGenZDataProcessor:
    ...
    def create_enhanced_training_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        def classify_text_enhanced(text: str) -> Tuple[str, Dict[str, Any]]:
            ...
            for category, keywords in malicious_patterns.items():
                ...
                if found_keywords:
                    detected_categories[category] = {"keywords": found_keywords, "score": category_score}
            ...
            for category, keywords in safe_patterns.items():
                ...
                if found_keywords:
                    safe_categories[category] = {"keywords": found_keywords, "score": category_score}
    ...
    def evaluate_model_performance(
        self, y_true: List[str], y_pred: List[str], y_proba: Optional[List[float]] = None
    ) -> Dict[str, Any]:
    ...
    def train_advanced_models(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        best_model_name = max(ensemble_results["results"].keys(), key=lambda x: ensemble_results["results"][x]["f1"])
