"""
Topic Modeling with Existing DistilBERT Integration

This module provides topic modeling capabilities using the existing DistilBERT
implementation from advanced_model.py. It avoids dependency issues by reusing
the existing transformer setup.
"""
...
class TopicAnalyzer:
    """
    Topic analyzer using existing DistilBERT implementation
    """
    ...
    def __init__(
        ...
        model_save_path: str = "topic_model.pkl",
    ...
    def generate_embeddings(self, texts: List[str], use_cache: bool = True, batch_size: int = 16) -> np.ndarray:
        """
            ...
            if texts_to_embed:
                ...
                for i in range(0, len(texts_to_embed), batch_size):
                    ...
                    with torch.no_grad():
                        ...
                        new_embeddings.extend(batch_embeddings.cpu().numpy())
    ...
    def create_clustering_model(self, embeddings: np.ndarray) -> Any:
        ...
        try:
            ...
            self.umap_model = UMAP(
                n_neighbors=15, n_components=min(5, embeddings.shape[1] - 1), min_dist=0.0, metric="cosine", random_state=42
            )
            ...
            self.clusterer = hdbscan.HDBSCAN(
                min_cluster_size=self.min_topic_size, min_samples=5, metric="euclidean", cluster_selection_method="eom"
            )
    ...
    def extract_topic_keywords(
        self, texts: List[str], topics: np.ndarray, top_k: int = 10
    ...
    def train_topic_model(self, texts: List[str], embeddings: Optional[np.ndarray] = None) -> Dict[str, Any]:
        """
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        if self.is_trained and self.topic_info is not None:
            info.update({"n_topics_found": len(self.topic_info), "model_save_path": self.model_save_path})

        return info


def analyze_malicious_posts_topics(
    csv_path: str, n_topics: int = 4, min_topic_size: int = 10, save_model: bool = True
) -> Dict[str, Any]:
    """
    ...
    try:
        # Initialize analyzer
        analyzer = TopicAnalyzer(n_topics=n_topics, min_topic_size=min_topic_size)
        ...
        for topic_id in range(results["n_topics_found"]):
            ...
            topic_summaries[f"topic_{topic_id}"] = {"keywords": keywords, "top_keywords": [kw[0] for kw in keywords[:5]]}
...
def test_topic_setup(sample_size: int = 50) -> Dict[str, Any]:
    """
    Test topic setup with sample data
    ...
    """
    try:
        logger.info(f"Testing topic setup with {sample_size} sample posts")

        # Initialize analyzer
        analyzer = TopicAnalyzer(n_topics=3, min_topic_size=5)
        ...
        logger.info("Topic setup test completed successfully")
        return test_results

    except Exception as e:
        logger.error(f"Topic setup test failed: {e}")
        return {"status": "failed", "error": str(e)}


if __name__ == "__main__":
    # Run test when script is executed directly
    test_results = test_topic_setup(sample_size=100)
    print("Test Results:")
